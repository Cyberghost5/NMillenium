...
class _LoadWebViewState extends State<LoadWebView>
          ...
          if (_validURL)
            InAppWebView(
              ...
              onGeolocationPermissionsShowPrompt: (controller, origin) async {
                // Location permission handled by system - no manual permission request
                return Future.value(
                  GeolocationPermissionShowPromptResponse(
                    origin: origin,
                    allow: false,  // Deny geolocation to avoid permission requirements
                    retain: false,
                  ),
                );
              },
              onPermissionRequest: (controller, request) async {
                // Camera and microphone permissions removed for Google Play compliance
                // Deny all permission requests to avoid triggering permission dialogs
              ...
              onDownloadStartRequest:
                ...
                // File downloads disabled to comply with Google Play policy
                // No storage permissions = no file downloads
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Downloads are not supported in this app'),
                  ),
                );
              },
  ...
  // Permission request methods removed for Google Play compliance
  // The app now relies on Android's system Photo Picker for file selection
  // No storage or media permissions are requested
}
